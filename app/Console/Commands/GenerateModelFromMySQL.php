...
class GenerateModelFromMySQL extends Command
{
    ...
    public function fire()
    {
        ...
        if (empty($matches[2])) {
            $matches[2] = env('DB_DATABASE');
        }    //No longer require the database name to be provided
        ...
        //Match the tables
        $this->tables = $this->getMatchingTables($database_name, $table_name);
        ...
        foreach ($this->tables as $table) {
            ...
            $template = preg_replace('/#TIMESTAMPS#/', $this->generateTimestamps($fields), $template);
...
\t}\n\n";
    ...
    private function generateTimestamps($table_fields)
    {
        // #TIMESTAMPS#
        $timestamps = false;
        foreach ($table_fields as $field) {
